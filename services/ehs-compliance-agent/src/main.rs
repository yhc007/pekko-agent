...
async fn health() -> Json<HealthResponse> {
    Json(HealthResponse { status: "healthy", agent_id: "ehs-compliance-agent", version: env!("CARGO_PKG_VERSION") })
}
...
async fn main() -> anyhow::Result<()> {
    ...
    let port: u16   = std::env::var("EHS_COMPLIANCE_PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(8083);

    info!(agent_id = "ehs-compliance-agent", %gateway_url, "Starting EHS Compliance Agent");
